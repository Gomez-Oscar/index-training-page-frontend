...
const UpdateSlide = () => {
  ...
  const identifiedSlide = DUMMY_SLIDES.find((s) => s.id === slideId);

  useEffect(() => {
    setFormData(
      {
        title: {
          value: identifiedSlide.title,
          isValid: true,
        },
        url: {
          value: identifiedSlide.url,
          isValid: true,
        },
      },
      true
    );
  }, [setFormData, identifiedSlide]);

  const slideUpdateSubmitHandler = (event) => {
    event.preventDefault();
    console.log(formState.inputs);
  };

  if (!identifiedSlide) {
    return <h2>could not find a slide</h2>;
  }
