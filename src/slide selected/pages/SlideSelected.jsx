import React from 'react';
import { useParams } from 'react-router';

import SlideSelectedItem from '../components/SlideSelectedItem';

const DUMMY_SLIDES = [
  {
    id: 's1',
    title: 'Inland',
    image:
      'data:image/png;base64,',
    slideUrl:
      'https://docs.google.com/presentation/d/e/2PACX-1vRlSoW6nt91h1Fxy4OkZZHjRGIVAJ7VMuJwUKt8G4a6xRxHYNRV4kSgpnVVXG363MP0MDuocCzRz-8M/pub?start=false&loop=false&delayms=3000&output=embed',
  },
  {
    id: 's2',
    title: 'XML',
    image:
      'https://tecnologia-facil.com/wp-content/uploads/2019/10/que-es-xml-1.jpg',
    slideUrl:
      'https://docs.google.com/presentation/d/e/2PACX-1vRnyN6fQsU9TgbO7POfCkljA872GplLAxDNgIz_5S7gYmj0y_miEpFeo_zPGUp6cw/pub?start=false&loop=false&delayms=3000',
  },
  {
    id: 's3',
    title: 'Button More',
    image:
      'https://cdn.hashnode.com/res/hashnode/image/upload/v1630782257908/ry89uIDvO.jpeg?w=1600&h=840&fit=crop&crop=entropy&auto=compress,format&format=webp',
    slideUrl:
      'https://docs.google.com/presentation/d/e/2PACX-1vTqeMOrQ4ziwIJommJx_Pea6xxZ7rt6UuGCF8zawGjA_lc8SMcZW8UwqKyOKPmGBQ/pub?start=false&loop=false&delayms=3000',
  },
  {
    id: 's4',
    title: 'JSON POST',
    image: 'https://www.nylas.com/wp-content/uploads/JSON_Blog_Hero.png',
    slideUrl:
      'https://docs.google.com/presentation/d/e/2PACX-1vSC8PNOXu5ZhRLdlZ9mWFLBdoWGns4veq5f91o1Rk7roLfXQLDJ0yKXBeoHFFMYkg/pub?start=false&loop=false&delayms=3000',
  },
  {
    id: 's5',
    title: 'Tickets System',
    image: 'https://www.itarian.com/images/ticketing-system.png',
    slideUrl:
      'https://docs.google.com/presentation/d/e/2PACX-1vS4yJlVOxiPVALSEjsi7MeITiFm_BIXxP2KTpkq1Eb3yWG2UeHNxxqUyZAhZ48cRA/pub?start=false&loop=false&delayms=3000',
  },
  {
    id: 's6',
    title: 'Scanid Overview',
    image:
      'https://www.elegantthemes.com/blog/wp-content/uploads/2018/04/asana-featured-image.png',
    slideUrl:
      'https://docs.google.com/presentation/d/e/2PACX-1vQwi-huG_b1IkBHpPhs4ej6lKLwYjRiIcy5Y_pS-Raj0rCiWP7Zb4lBAiOR7bO53Q/pub?start=false&loop=false&delayms=3000',
  },
];

const SlideSelected = () => {
  const { slideId } = useParams();
  const loadedSlides = DUMMY_SLIDES.filter((slide) => slide.id === slideId);
  return <SlideSelectedItem items={loadedSlides} />;
};

export default SlideSelected;
